...
* Date: May 1, 2024
* Title Lab 5 - Server
* Description: UDP server that reads packets from a client and write to destination file.
* Packets include a checksum, sequence number, and acknowledgement number
*/
...
Packet serverReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen, int seqnum) {
    Packet packet;
    while (1) { //keep trying to receive packets until receive good packet
        ...
        if (packet.header.cksum!=e_cksum) {
            ...
            //resend packet (bad acknowledgement, so client resends --> read with recvfrom again in next loop iteration)
        ...
        //if the ckechsum and sequence numbers are correct send ack with the right sequence number
            ...
            //send ack (with correct seqnum)
            serverSend(sockfd, address, *addrlen, seqnum);
            break;
        }
    }
    return packet; //return good received packet
}
...
int main(int argc, char *argv[]) {
    ...
    socklen_t serverAddrSize = sizeof(servAddr);

    //bind IP address and port for server endpoint socket
    bind(sockfd, (struct sockaddr*)&servAddr, serverAddrSize);
    ...
    do {
        ...
        packet = serverReceive(sockfd, (struct sockaddr*)&servAddr, &serverAddrSize, seqnum);
