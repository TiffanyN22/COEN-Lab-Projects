...
* Description: This program implements a UDP socket across 4 servers
* and calculates the cost to send data to each of the other servers
...
void * receive_info (void *arg)
{
	...
	while (1) {
        //recv packets 
	    recvfrom(sock, (void *) &packet, sizeof(packet), 0, (struct sockaddr *) &addr, &addr_size);
